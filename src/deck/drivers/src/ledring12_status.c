...
static bool update()
{
  if(effect > neffect) return false;
  ledring12Worker(NULL);
  setHeadlightsOn(headlightEnable);
  return true;
}

static void setColor(uint8_t *color, uint8_t index)
{
  static uint8_t buffer[NBR_LEDS][3];
  int i;
  for (i=0; i<NBR_LEDS; i++)
  {
    memcpy(buffer[i], color, 3);
  }
  ws2812Send(buffer, NBR_LEDS);
}

static statusLed_t led = {&setColor, &update, true, 20};
...
PARAM_ADD(PARAM_UINT8, useForStatus, &(led.active))
PARAM_ADD(PARAM_UINT8, brightness, &(led.brightness))
